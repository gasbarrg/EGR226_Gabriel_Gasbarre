...
int Keypad_Value(void){
    /****| Keypad_Value | *****************************************
    * Brief: Reads keystrokes to allow three-digit integer
    *  inputs, up to a maximum of 100. positive integers only.
    *
    * param: N/A
    *
    * return:
    ...
    *************************************************************/
    ...
    while(Keypad_Read() == 404){}
        if (Keypad_Read() == 11){
            ...
            //Move value over by one base 10
            val = val * 10;
            //save new value
            val = val + Keypad_Read();
        }
    ...
    //Repeat above steps for final base(can only be 100 or input error)
    while(Keypad_Read() == 404){}
        if (Keypad_Read() == 11){
            return val;
        }
...
void KeypadWaitUntilLow (void)
{
    while (Keypad_Read() != 404);
}

void KeypadWaitUntilHigh (void)
{
    while (Keypad_Read() == 404);
}
